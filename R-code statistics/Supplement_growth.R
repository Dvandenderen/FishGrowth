...
### FishBase growth data statistics, supplementary 
####################################################
...
###### combine data into dataframe
  growth_groups_Tzeroone <- data.frame(Guild = NA, obs = NA, Model = NA, BestMod = NA, Intercept = NA, Temperature=NA, Preybio = NA, Pvalue_temp = NA, Pvalue_food=NA, Q10 = NA, note=NA)
  growth_groups_Tzeroone[1,] <- c("Large pelagics", nrow(lpel), as.character(Lpelmod$call)[1:2], coef(Lpelmod)[1], NA,coef(Lpelmod)[2],NA, summary(Lpelmod)$coefficients[2,4], Q10_Lpel,"Q10 is calculated from the model: Linf ~ Temp + Prey. Inclusion of temperature does not improve model fit, the temperature effect is calculated for average food concentrations")  
  growth_groups_Tzeroone[2,] <- c("Small pelagics", nrow(spel), as.character(spelmod$call)[1:2], coef(spelmod), NA, summary(spelmod)$coefficients[2,4],NA, Q10_spel,"-")  
  growth_groups_Tzeroone[3,] <- c("Sharks and rays", nrow(shray), as.character(shraymodel$call)[1:2], coef(shraymodel), NA, summary(shraymodel)$coefficients[2,4],NA, Q10_shray,"-")  
  growth_groups_Tzeroone[4,] <- c("Large demersal", nrow(ldem), as.character(ldemmod$call)[1:2], coef(ldemmod), NA, summary(ldemmod)$coefficients[2,4],NA, Q10_ldem,"-")  
  growth_groups_Tzeroone[5,] <- c("Small demersal", nrow(sdem), as.character(sdemmod$call)[1:2], coef(sdemmod),NA, summary(sdemmod)$coefficients[2,4],NA, Q10_sdem,"-")  
  growth_groups_Tzeroone[6,] <- c("Deep-living fish",nrow(deep), as.character(deepmodel$call)[1:2], coef(deepmodel), NA, summary(deepmodel)$coefficients[2,4],NA, Q10_deep,"-")  
...
###### combine data into dataframe
  growth_groups_uniq <- data.frame(Guild = NA, obs = NA, Model = NA, BestMod = NA, Intercept = NA, Temperature=NA, Preybio = NA, Pvalue_temp = NA, Pvalue_food=NA, Q10 = NA,note=NA)
  growth_groups_uniq[1,] <- c("Large pelagics", nrow(lpel), as.character(Lpelmod$call)[1:2], coef(Lpelmod),NA, summary(Lpelmod)$coefficients[2,4],NA, Q10_Lpel,"-")  
  growth_groups_uniq[2,] <- c("Small pelagics", nrow(spel), as.character(spelmod$call)[1:2], coef(spelmod), NA, summary(spelmod)$coefficients[2,4],NA, Q10_spel,"-")  
  growth_groups_uniq[3,] <- c("Sharks and rays", nrow(shray), as.character(shraymodel$call)[1:2], coef(shraymodel), NA, summary(shraymodel)$coefficients[2,4],NA, Q10_shray,"-")  
  growth_groups_uniq[4,] <- c("Large demersal", nrow(ldem), as.character(ldemmod$call)[1:2], coef(ldemmod), NA, summary(ldemmod)$coefficients[2,4],NA, Q10_ldem,"-")  
  growth_groups_uniq[5,] <- c("Small demersal", nrow(sdem), as.character(sdemmod$call)[1:2], coef(sdemmod),NA, summary(sdemmod)$coefficients[2,4],NA, Q10_sdem,"-")  
  growth_groups_uniq[6,] <- c("Deep-living fish",nrow(deep), as.character(deepmodel$call)[1:2], coef(deepmodel), NA, summary(deepmodel)$coefficients[2,4],NA, Q10_deep,"-")  
