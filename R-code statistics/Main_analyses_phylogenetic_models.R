...
# run the phylogenetic model (M4) for data subst (fish_phylo)
  ...
  burnin <- 1000
  thin <- nitt/1000
  M4_phylo <- MCMCglmm(LArate ~ T_within + T_across,random=~us(1+T_within):Name + uniReg,
...
# compare with model without phylogeny based on data subset (fish_phylo)
  ...
  burnin <- 1000
  thin <- nitt/1000
  M4_nophylo <- MCMCglmm(LArate ~ T_within + T_across, random=~us(1 + T_within):Name + uniReg,
                        data=fish_phylo, prior = prior_Ps,
                        nitt=nitt, burnin=burnin,
                        thin=thin, verbose=FALSE)
  summary(M4_nophylo) 
...
# run M5 with phylogenetic distance matrix resulting in appendix 2, table A2-5 last column
  nitt <- 5e4
  burnin <- 1000
  thin <- nitt/1000
  ...
  temp <- c(10,20)
  # Pel 30 cm
  yq10 <- 0.026274*temp + 0.005768*temp + -0.011440*log10(30)*temp
  10^(yq10[2])/10^(yq10[1])
  
  # Pel 100 cm
  yq10 <- 0.026274*temp + 0.005768*temp + -0.011440*log10(100)*temp
  10^(yq10[2])/10^(yq10[1])

  
  # Dem 30 cm
  yq10 <- 0.026274*temp + 0.001867*temp + -0.011440*log10(30)*temp
  10^(yq10[2])/10^(yq10[1])
  
  # Dem 100 cm
  yq10 <- 0.026274*temp + 0.001867*temp + -0.011440*log10(100)*temp
  10^(yq10[2])/10^(yq10[1])
  ...
  # Deep 30 cm
  yq10 <- 0.026274*temp + -0.011440*log10(30)*temp
  10^(yq10[2])/10^(yq10[1])
  
  # Deep 100 cm
  yq10 <- 0.026274*temp + -0.011440*log10(100)*temp
  10^(yq10[2])/10^(yq10[1])
